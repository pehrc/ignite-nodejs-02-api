...
async function transactionsRoutes(app2) {
  app2.get("/", { preHandler: [checkSessionIdExists] }, async (req) => {
  ...
  app2.get("/summary/all", async () => {
    const summary = await knex("transactions").sum("amount", { as: "amount" }).first();
    return { summary };
  });
  app2.get("/:id", { preHandler: [checkSessionIdExists] }, async (req) => {
  ...
  app2.get("/summary", { preHandler: [checkSessionIdExists] }, async (req) => {
    const { sessionId } = req.cookies;
